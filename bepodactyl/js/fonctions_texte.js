...
function aff_result() // replacer les undefined par des espaces insécables (pour l'affichage)
{
  ...
  if(nb_err > 5)
	...
	if (nb_err == 0)
    document.getElementById("resultats").innerHTML = "Félicitations ! Vous n'avez fait aucune erreur ! En "+compteur_temps+"seconde ! ";
  else if (nb_err < 2)
    document.getElementById("resultats").innerHTML = "Vous avez fait "+nb_err+" erreur. En "+compteur_temps+"seconde ! ";
	else
    document.getElementById("resultats").innerHTML = "Vous avez fait "+nb_err+" erreurs. En "+compteur_temps+"seconde ! "+txt;
	document.getElementById("rd_txt").innerHTML = "";
...
{
  var clock="&#8987; ";
	var chronometre = document.getElementById("chronometre");
	var date_evenement = new Date();
	var total_secondes = (date_evenement - date_actuelle) / 1000;
    minutes = Math.floor(total_secondes / 60);
		secondes = Math.floor(total_secondes - minutes * 60);

		chronometre.innerHTML = minutes+ ' min '+secondes +'s <span style="color:black;font-size: .6em;">'+clock+'</span>';
	var actualisation = setTimeout("chronometre();", 1000);
}
